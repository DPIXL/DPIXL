# About me
- I'm Štěpán Václav Patejdl, a Czech IT student and Open-Source software enthusiast, known on the internet as DPIXL ("deadpixl", sometimes stylized as P!XL, P1XL, etc...)
- 19 years old
- Started programming in c# (and also using Linux) during the pandemic
- SSŠVT student, in the final year
- A lot of the projects here are school projects, like boty-asp (ASP .NET MVC project) or CarTrackBuilder (a simple HTML/JS/CSS project) or UML_forms

## My Projects
### Released
- WWHCS / WWHCS-console : A .NET windows registry editor, changing the color of your window header. It can set different colors for either active or inactive.
### Ongoing
- pixl_md_editor : My very own Markdown editor, Avalonia project, I am making it mainly for personal use. Im mainly going to make it for Linux, since that's the OS I use, but I'll consider Windows compatibility if someone requests it.
### Old
- CubeLab : My first ever project, a 2D logic/rage platformer, programmed in C# with the Unity Engine. It isn't very well made, and only the scripts are here. Dont look at it please. Never finished it sadly, but I am planning to remake it with my girlfriend soon.
